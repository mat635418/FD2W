...
def load_and_process_data(file_source):
    ...
    # UNIVERSAL UNPIVOT (Works on all Pandas versions)
    df_long = df_pivot.stack(level=[0, 1]).reset_index()
    # The last column generated by stack is the volume values
    df_long.rename(columns={df_long.columns[-1]: 'Volume'}, inplace=True)
    ...
    # Fuzzy mapping to avoid exact string matching errors
    def map_role(val):
        val = str(val).lower()
...
def load_locations_and_geocode(file_source, limit):
    ...
    # Limit rows to speed up rendering
    df_loc = df_loc.head(limit).copy()
    
    headers = {'User-Agent': 'GoodYearDistributionApp/1.3'}
    ...
    with col1:
        ...
        if st.button("Load pre-selected FD2W data"):
            if os.path.exists(pivot_name) and os.path.exists(loc_name):
                ...
                st.rerun() # FORCES THE UI TO UPDATE
            else:
...
# Error Catcher: If anything breaks here, it prints to the screen!
try:
